...
export async function POST(request: NextRequest, context: RouteContext) {
  try {
    ...
    if (
      ...
      return Response.json({ error: "Выбери друга для public-приглашения." }, { status: 400 });
    }

    const store = await getCollaborationStore();
    const item = await store.inviteToPublicCategory({
    ...
    return Response.json(
      { error: toErrorMessage(error, "Не удалось отправить public-приглашение.") },
      { status: 500 }
    );
  }
}
