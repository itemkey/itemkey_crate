...
export async function GET(request: NextRequest, context: RouteContext) {
  try {
    ...
    const inbox = await store.listInboxForFriend(user.id, id);
    return Response.json({ data: inbox, source: store.source });
  } catch (error) {
    return Response.json(
      { error: toErrorMessage(error, "Не удалось загрузить inbox.") },
      { status: 500 }
    );
  }
}
